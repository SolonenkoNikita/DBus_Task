...
ConfigApplication::ConfigApplication() : 
        config_file_path_(expandPath("~/" + DEFAULT_CONFIG_DIR + DEFAULT_CONFIG_FILE)) 
{
...
void ConfigApplication::loadInitialConfig() 
{
    try 
    {
        ...
        std::cout << "Loaded config - Timeout: " << timeout_.count() 
                  << "ms, Phrase: '" << timeout_phrase_ << "'\n";
    } 
...
void ConfigApplication::setupDBusConnection() 
{
    try 
    {
        ...
        std::cout << "Creating proxy for service: " << DBUS_SERVICE 
                  << ", object: " << DBUS_OBJECT << '\n';
        dbus_proxy_ = sdbus::createProxy(*connection_, DBUS_SERVICE, DBUS_OBJECT);
        
        std::cout << "Subscribing to configuration changes...\n";
        dbus_proxy_->uponSignal("configurationChanged").onInterface(DBUS_INTERFACE)
