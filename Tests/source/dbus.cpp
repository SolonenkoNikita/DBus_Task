...
class MockConfigStorage : public IConfigStorage
{
   public:
    ...
    std::map<std::string, sdbus::Variant> getAllParameters() const override { return parameters_; }
    ...
    void setParameters(const std::map<std::string, sdbus::Variant>& params) { parameters_ = params; }
...
{
    auto proxy =
        sdbus::createProxy(*connection_, "test.config.manager", "/com/system/configurationManager/Application/testApp");
...
TEST_F(DBusConfigAdapterTest, EmitsConfigurationChangedSignal)
{
    ...
    proxy->uponSignal("configurationChanged")
        .onInterface("com.system.configurationManager.Application.Configuration")
        .call(
            [&](const std::map<std::string, sdbus::Variant>& config)
            {
